// Build a print-ready HTML bill document, styled after the paper ledger
// bills this system replaces (AJAY statement + B-7 ledger + GST template).
...
function billHtml(bill) {
  const itemRows = (bill.items || []).map((it) => `
        <tr>
          <td>${esc(it.trip_date ? new Date(it.trip_date).toLocaleDateString('en-IN') : '—')}</td>
          <td>${esc(it.particulars)}</td>
          <td>${esc(it.vehicle_registration || '—')}</td>
          <td class="num">${esc(it.rate_basis || '')}</td>
          <td class="num">${fmt(it.amount)}</td>
          <td class="num">${it.advance ? fmt(it.advance) : '—'}</td>
        </tr>`).join('');

  const paymentRows = (bill.payments || []).map((p) => `
        <tr>
          <td>${esc(new Date(p.payment_date).toLocaleDateString('en-IN'))}</td>
          <td>${esc(p.mode)}</td>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sign-line { border-top: 1px solid #1a1a1a; width: 220px; padding-top: 6px; }
...
<body>
  <div class="wrap">
    <div class="head">
      <div>
        <div class="company">${esc(bill.company_name)}</div>
        <div class="muted">${esc(bill.company_address || '')}</div>
        <div class="muted">${bill.company_gstin ? 'GSTIN: ' + esc(bill.company_gstin) : ''}${bill.company_phone ? ' · ' + esc(bill.company_phone) : ''}</div>
      </div>
      <div class="bill-meta">
        <div class="bill-no">${esc(bill.bill_no)}</div>
        <div>Date: ${esc(new Date(bill.bill_date).toLocaleDateString('en-IN'))}</div>
    ...
    <div class="payments">
      ...
      <table>
        <thead><tr><th style="width:110px">Date</th><th>Mode</th><th class="num" style="width:130px">Amount</th></tr></thead>
    ...
    <div class="sign">
      ...
      <div class="sign-line" style="text-align:right">For ${esc(bill.company_name || '')} — Authorised Signatory</div>
    </div>
  </div>
</body>
</html>`;
